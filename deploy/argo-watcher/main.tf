locals {
  postgres_cluster_name = "argo-watcher-psql"
  postgres_user         = "watcher"

  # Generated by the operator from its default secret_name_template
  # "{username}.{cluster}.credentials.{tprkind}.{tprgroup}"
  postgres_secret_name = "${local.postgres_user}.${local.postgres_cluster_name}.credentials.postgresql.acid.zalan.do"
...
resource "kubernetes_manifest" "postgres" {
  ...
  manifest = yamldecode(templatefile("${path.module}/templates/argo-watcher-psql.tftpl", {
    ...
    rawChartVersion = var.raw_chart_version
    clusterName     = local.postgres_cluster_name
    teamId          = var.postgres_team_id
...
resource "kubernetes_manifest" "this" {
  manifest = yamldecode(templatefile("${path.module}/templates/argo-watcher.tftpl", {
    ...
    postgresHost             = local.postgres_cluster_name
    postgresUser             = local.postgres_user
